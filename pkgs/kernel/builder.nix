  ...
  perSystem =
    { pkgs, lib, ... }:
    {
      lib.buildKernel =
        ...
        let
          evaluatedStructuredConfig = import ./_eval-config.nix {
            inherit
              lib
              path
              writeShellScript
          ...
          kernelDerivation = stdenv.mkDerivation (
            ...
            // {
              ...
              patches =
                ...
                ++ optional (lib.versionAtLeast version "4.13" && lib.versionOlder version "5.19") (
                  nixosKernelPath + "/randstruct-provide-seed.patch"
                )
                ++ optional (lib.versionAtLeast version "5.19") (
                  nixosKernelPath + "/randstruct-provide-seed-5.19.patch"
                )
                ++ optional (
                  enableDefaultYYLOCPatch && lib.versionOlder version "4.0"
                ) ./gcc10-extern_YYLOC_global_declaration.patch
                ++ optional (enableForceLogoPatch && lib.versionAtLeast version "5.4") (fetchpatch {
              ...
              passthru =
                let
                  ...
                  configUpdater =
                    attrs:
                    kernelDerivation.overrideAttrs (
                      { ... }:
                      (
                        {
                ...
                in
                {
                  ...
                  menuconfig = kernelDerivation.overrideAttrs (
                    {
                      nativeBuildInputs ? [ ],
                      ...
                    }:
